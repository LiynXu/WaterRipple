...
- (void)drawRect:(CGRect)rect {
    ...
    self.rippleDisplayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(getCurrentRipple)];
    [self.rippleDisplayLink addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
}
...
- (void)drawMainRipple{
    ...
    for (float x = 0.f; x <= self.rippleWidth ; x++) {
        y = self.rippleAmplitude * sin((180 / self.rippleWidth *  M_PI/150 )* x   - self.mainRippleoffsetX *M_PI/180) + self.ripplePosition;
...
- (void)drawMinorRipple{
    ...
    CGFloat y = 0.f;
    for (float x = 0.f; x <= self.rippleWidth ; x++) {
        y = self.rippleAmplitude * sin((180 / self.rippleWidth *  M_PI/150 )*x  - self.minorRippleoffsetX*M_PI/360 ) + self.ripplePosition;
